...
export class QuotesRepository {
    ...
    const [rows] = await pool.query(
      `
        SELECT
          ...
          q.provider_proposed_date,
          q.provider_proposed_time_range,
          q.sent_at,
          q.accepted_at,
          q.rejected_at,
          q.expires_at,
  ...
  static async updateProposal(
    ...
    const [result] = await connection.execute(
      `
        UPDATE quotes
           SET proposal_amount = ?,
               ...
               provider_proposed_date = ?,
               provider_proposed_time_range = ?,
      ...
      [
        ...
        payload.suggestedDate || null,
        payload.suggestedTimeRange || null,
  ...
  private static mapListRow(row: any): QuoteListRecord {
    return {
      ...
      provider_proposed_date: row.provider_proposed_date ? String(row.provider_proposed_date) : null,
      provider_proposed_time_range: row.provider_proposed_time_range ?? null
    };
  }
