    ...
    try {
      ...
      for (const column of columnDefinitions) {
        ...
        if (!exists) {
          ...
          Logger.info(MODULE, '[CASH_SCHEMA] Columna aÃ±adida', { column: column.name });
        }
      }
      ...
      for (const index of indexDefinitions) {
        ...
        if (!exists) {
          ...
          Logger.info(MODULE, '[CASH_SCHEMA] Ãndice creado', { index: index.name });
        }
      }

      await pool.query(
        `INSERT INTO platform_settings (setting_key, setting_value, setting_type, description)
         SELECT 'cash_max_amount', '150000', 'number', 'Tope mÃ¡ximo por cita para pagos en efectivo (CLP)'
...
function buildRouter(): Router {
  ...
  router.post('/appointments', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      const pool = DatabaseConnection.getPool();
      // Validar service -> provider y obtener precio/duraciÃ³n
      ...
      if ((sv as any[]).length === 0 || (sv as any[])[0].provider_id !== Number(provider_id)) {
        return res.status(400).json({ success: false, error: 'Servicio invÃ¡lido para el proveedor' });
      }
      const service = (sv as any[])[0];

      // Construir direcciÃ³n base desde perfil del cliente
      ...
      try {
        ...
        Logger.warn(MODULE, 'No se pudo obtener direcciÃ³n del cliente para la cita', profileErr as any);
      }
      ...
      console.log('ðŸŸ¢ [APPOINTMENTS] ==================== NUEVA CITA CREADA ====================');
      console.log('ðŸŸ¢ [APPOINTMENTS] Appointment ID:', id);
      console.log('ðŸŸ¢ [APPOINTMENTS] Provider ID:', provider_id);
      console.log('ðŸŸ¢ [APPOINTMENTS] Client ID:', client_id);
      console.log('ðŸŸ¢ [APPOINTMENTS] Client Name:', (appointment as any).client_name);
      console.log('ðŸŸ¢ [APPOINTMENTS] Start Time:', start_time);
      console.log('ðŸŸ¢ [APPOINTMENTS] Appointment Data:', JSON.stringify(appointment, null, 2));
      
      // Emitir en tiempo real a provider y client
      console.log('ðŸ”µ [APPOINTMENTS] Emitiendo socket a provider y client...');
      try { 
        emitToUser(provider_id, 'appointment:created', appointment);
        console.log('ðŸ”µ [APPOINTMENTS] âœ… Socket emitido a provider:', provider_id);
      } catch (err) {
        console.error('ðŸ”´ [APPOINTMENTS] âŒ Error emitiendo socket a provider:', err);
      }
      
      try { 
        emitToUser(client_id, 'appointment:created', appointment);
        console.log('ðŸ”µ [APPOINTMENTS] âœ… Socket emitido a client:', client_id);
      } catch (err) {
        console.error('ðŸ”´ [APPOINTMENTS] âŒ Error emitiendo socket a client:', err);
      }
      
      // Push al proveedor
      console.log('ðŸŸ£ [APPOINTMENTS] ==================== ENVIANDO PUSH NOTIFICATION ====================');
      const locationForMessage = (appointment as any).client_location_label || (appointment as any).client_location || locationLabel || '';
      const basePushMessage = `Cliente: ${(appointment as any).client_name || ''} â€¢ ${String(start_time).slice(0,5)}`;
      const pushMessage = locationForMessage ? `${basePushMessage} â€¢ ${locationForMessage}` : basePushMessage;

      console.log('ðŸŸ£ [APPOINTMENTS] Provider ID para push:', provider_id);
      console.log('ðŸŸ£ [APPOINTMENTS] TÃ­tulo:', 'Nueva cita por confirmar');
      console.log('ðŸŸ£ [APPOINTMENTS] Mensaje:', pushMessage);
      ...
      try { 
        ...
        console.log('ðŸŸ£ [APPOINTMENTS] âœ… Push notification enviada exitosamente');
      } catch (pushErr) {
        console.error('ðŸ”´ [APPOINTMENTS] âŒ Error enviando push notification:', pushErr);
        console.error('ðŸ”´ [APPOINTMENTS] Error stack:', (pushErr as Error).stack);
      }
      ...
      console.log('ðŸŸ¢ [APPOINTMENTS] ==================== FIN CREACIÃ“N CITA ====================');
  ...
  // GET /appointments/by-day?date=YYYY-MM-DD - listar por dÃ­a (proveedor actual)
  router.get('/appointments/by-day', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      const pool = DatabaseConnection.getPool();
      Logger.info(MODULE, `ðŸ“… Listando citas del dÃ­a para provider=${user.id} date=${date}`);
      const [rows] = await pool.query(
        `SELECT a.*,
                u.name AS client_name,
                ps.name AS service_name,
      ...
      Logger.info(MODULE, `ðŸ“… Citas retornadas: ${(rows as any[]).length}`, { sample: (rows as any[])[0] });
      res.setHeader('Cache-Control', 'no-store');
      return res.json({ success: true, appointments: rows });
    } catch (err) {
      Logger.error(MODULE, 'Error listing appointments by day', err as any);
      return res.status(500).json({ success: false, error: 'Error al listar citas del dÃ­a' });
    }
  });
  ...
  // PUT /appointments/:id/location - actualizar direcciÃ³n de la cita (proveedor)
  router.put('/appointments/:id/location', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      Logger.error(MODULE, 'Error actualizando direcciÃ³n de cita', err as any);
      return res.status(500).json({ success: false, error: 'Error al actualizar direcciÃ³n de la cita' });
    }
  });
  ...
  router.get('/availability/time-slots', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      console.log('ðŸŸ¢ [TIME_SLOTS] ==================== OBTENIENDO TIME SLOTS ====================');
      console.log('ðŸŸ¢ [TIME_SLOTS] Provider ID:', provider_id);
      console.log('ðŸŸ¢ [TIME_SLOTS] Date:', date);
      console.log('ðŸŸ¢ [TIME_SLOTS] Service ID:', service_id);
      ...
      console.log('ðŸŸ¢ [TIME_SLOTS] DuraciÃ³n del servicio:', duration, 'minutos');
      
      // Obtener dÃ­a de la semana
      const dayOfWeek = getDayOfWeekFromDate(date);
      console.log('ðŸŸ¢ [TIME_SLOTS] DÃ­a de la semana:', dayOfWeek);
      ...
      let blocks = (weekly as any[]).length > 0 
        ...
        : [{ start: '09:00', end: '18:00' }]; // Default si no hay configuraciÃ³n
      
      console.log('ðŸŸ¢ [TIME_SLOTS] Bloques semanales:', blocks);
      
      // 2. Verificar excepciones para esta fecha especÃ­fica
      const [exceptions] = await pool.query(
        'SELECT is_available, start_time, end_time, reason FROM availability_exceptions WHERE provider_id = ? AND exception_date = ?',
        [provider_id, date]
      );
      
      console.log('ðŸŸ¢ [TIME_SLOTS] Excepciones encontradas:', (exceptions as any[]).length);
      
      let blockedRanges: Array<{ start: string; end: string; reason: string }> = [];
      
      for (const exc of (exceptions as any[])) {
        console.log('ðŸŸ¢ [TIME_SLOTS] ExcepciÃ³n:', exc);
        
        if (!exc.is_available) {
          // Es un bloqueo
          if (!exc.start_time && !exc.end_time) {
            // Bloqueo de todo el dÃ­a
            console.log('ðŸ”´ [TIME_SLOTS] âš ï¸ TODO EL DÃA BLOQUEADO');
            blocks = []; // No hay bloques disponibles
          } else {
            // Bloqueo de horario especÃ­fico
            blockedRanges.push({
              start: String(exc.start_time).slice(0, 5),
              end: String(exc.end_time).slice(0, 5),
              reason: exc.reason || 'Bloqueado'
            });
            console.log('ðŸ”´ [TIME_SLOTS] Horario bloqueado:', exc.start_time, '-', exc.end_time);
          }
        } else {
          // Es una habilitaciÃ³n especial (sobrescribe horario semanal)
          if (exc.start_time && exc.end_time) {
            blocks = [{
              start: String(exc.start_time).slice(0, 5),
              end: String(exc.end_time).slice(0, 5)
            }];
            console.log('ðŸŸ¢ [TIME_SLOTS] Horario especial habilitado:', exc.start_time, '-', exc.end_time);
          }
        }
      }
      ...
      console.log('ðŸŸ¢ [TIME_SLOTS] Citas existentes:', (apps as any[]).length);
      ...
      for (const b of blocks) {
        ...
        while (addMinutes(cursor, duration) <= b.end) {
          ...
          // Verificar si estÃ¡ en un rango bloqueado
      ...
      console.log('ðŸŸ¢ [TIME_SLOTS] Slots generados:', slots.length);
      console.log('ðŸŸ¢ [TIME_SLOTS] Disponibles:', slots.filter(s => s.is_available).length);
      console.log('ðŸŸ¢ [TIME_SLOTS] Bloqueados:', slots.filter(s => s.reason === 'blocked').length);
      console.log('ðŸŸ¢ [TIME_SLOTS] Ocupados:', slots.filter(s => s.reason === 'booked').length);
      
      return res.json({ success: true, time_slots: slots });
    } catch (err) {
      console.error('ðŸ”´ [TIME_SLOTS] Error:', err);
  ...
  // GET /client/appointments - listar citas del cliente autenticado (prÃ³ximas/pasadas/canceladas)
  ...
  // PATCH /appointments/:id/status - actualizar sÃ³lo el estado
  router.patch('/appointments/:id/status', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      const pool = DatabaseConnection.getPool();
      // Asegurar autorizaciÃ³n: proveedor dueÃ±o o cliente dueÃ±o puede cambiar a cancelada; confirmado/completed solo provider
      ...
      // Enviar notificaciones push segÃºn el estado
      try {
        if (status === 'confirmed') {
          await PushService.notifyUser(
  ...
  router.post('/appointments/:id/cash/collect', authenticateToken, cashClosureGate, async (req: Request, res: Response) => {
    try {
      ...
      if (!Number.isFinite(appointmentId)) {
        Logger.warn(MODULE, '[CASH][SELECT] id invÃ¡lido recibido', { rawId: req.params.id });
        return res.status(400).json({ success: false, error: 'id invÃ¡lido' });
      }
      ...
      // Solo el proveedor dueÃ±o puede registrar cobro en efectivo
      ...
      // Obtener tax y comisiÃ³n desde settings
      ...
      // Registrar deuda de comisiÃ³n del proveedor (si existe la tabla) con due_date (T + cash_commission_due_days)
      try {
        ...
        Logger.warn(MODULE, 'No se pudo registrar deuda de comisiÃ³n (tabla puede no existir aÃºn)');
      }

      // Actualizar appointment para reflejar mÃ©todo de pago
      try {
      ...
      try {
        await PushService.notifyUser(
          ...
          'El proveedor registrÃ³ el cobro de tu cita en efectivo.',
  ...
  // POST /appointments/:id/cash/select - seleccionar efectivo y generar cÃ³digo
  router.post('/appointments/:id/cash/select', authenticateToken, cashClosureGate, async (req: Request, res: Response) => {
    try {
      ...
      const amount = Number(appt.price || 0);
      if (!(amount > 0)) {
        Logger.warn(MODULE, '[CASH][SELECT] precio invÃ¡lido para la cita', { appointmentId, amount });
        return res.status(400).json({ success: false, error: 'Precio invÃ¡lido para la cita' });
      }
      ...
      // Generar/asegurar cÃ³digo de verificaciÃ³n
      ...
      if (!verificationCode) {
        ...
        try {
          ...
          return res.status(500).json({ success: false, error: 'No se pudo guardar el cÃ³digo de verificaciÃ³n. Intenta nuevamente.' });
        }
      } else {
        try {
          await pool.execute(`UPDATE appointments SET payment_method = 'cash', updated_at = NOW() WHERE id = ?`, [appointmentId]);
  ...
  // POST /appointments/:id/cash/verify-code - validar cÃ³digo para cierre/pago cash
  router.post('/appointments/:id/cash/verify-code', authenticateToken, cashClosureGate, async (req: Request, res: Response) => {
    try {
      ...
      const { code } = req.body || {};
      if (!Number.isFinite(appointmentId)) {
        Logger.warn(MODULE, '[CASH][VERIFY] id invÃ¡lido recibido', { rawId: req.params.id });
        return res.status(400).json({ success: false, error: 'id invÃ¡lido' });
      }

      const cleanCode = sanitizeCode(String(code ?? ''));
      if (!validateCodeFormat(cleanCode)) {
        Logger.warn(MODULE, '[CASH][VERIFY] formato de cÃ³digo invÃ¡lido', { appointmentId, cleanCode });
        return res.status(400).json({ success: false, error: 'CÃ³digo invÃ¡lido' });
      }

      Logger.info(MODULE, '[TRACE][VERIFY] Solicitud validar cÃ³digo cash', {
        appointmentId,
        providerRequestId: user.id,
      ...
      if (Number(appt.provider_id) !== Number(user.id)) {
        ...
        return res.status(403).json({ success: false, error: 'Solo el proveedor puede validar el cÃ³digo' });
      }

      // Ventana de validaciÃ³n: desde inicio hasta fin + 90min (configurable posteriormente)
      // Nota: aquÃ­ solo aplicamos verificaciÃ³n de cÃ³digo y tope cash
      ...
      if (!stored || !compareVerificationCodes(cleanCode, stored)) {
        Logger.warn(MODULE, '[CASH][VERIFY_CODE] CÃ³digo incorrecto', {
      ...
      try {
        ...
        Logger.warn(MODULE, '[CASH] No se pudo registrar deuda de comisiÃ³n durante verify-code', err as any);
      }
      ...
      try {
        await PushService.notifyUser(
          ...
          'El proveedor validÃ³ el cÃ³digo de tu cita y registrÃ³ el pago en efectivo.',
          { type: 'payment', appointment_id: String(appointmentId), method: 'cash' }
        );
      } catch {}
      ...
      return res.status(500).json({ success: false, error: 'Error al verificar cÃ³digo de efectivo' });
    }
  });

  // Helper: resolver cierre mutuo si hay suficiente informaciÃ³n
  async function resolveClosureIfPossible(appointmentId: number) {
    ...
    // Si el cliente dijo OK o no hay acciones pero queremos forzar comisiÃ³n por defecto, crear payment+debt
  ...
  router.post('/appointments/:id/closure/provider-action', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      if (!Number.isFinite(appointmentId)) return res.status(400).json({ success: false, error: 'id invÃ¡lido' });
      if (!['no_show','issue','code_entered'].includes(String(action))) return res.status(400).json({ success: false, error: 'acciÃ³n invÃ¡lida' });
  ...
  router.post('/appointments/:id/closure/client-action', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      const { action, notes } = req.body || {};
      if (!Number.isFinite(appointmentId)) return res.status(400).json({ success: false, error: 'id invÃ¡lido' });
      if (!['ok','no_show','issue'].includes(String(action))) return res.status(400).json({ success: false, error: 'acciÃ³n invÃ¡lida' });
  ...
  /**
   * Obtener dÃ­a de la semana desde fecha YYYY-MM-DD
   * @param dateStr Fecha en formato YYYY-MM-DD
   * @returns DÃ­a de la semana: 'monday', 'tuesday', etc.
   */
  ...
  // ========================================
  // ENDPOINTS DE VERIFICACIÃ“N DE CÃ“DIGOS
  // ========================================

  /**
   * POST /appointments/:id/verify-completion
   * Verifica el cÃ³digo de 4 dÃ­gitos para marcar servicio como completado
  ...
  router.post('/appointments/:id/verify-completion', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      Logger.info(MODULE, 'ðŸ” ==================== VERIFICANDO CÃ“DIGO ====================');
      Logger.info(MODULE, `ðŸ” Appointment ID: ${appointmentId}`);
      Logger.info(MODULE, `ðŸ” Provider ID: ${user.id}`);
      Logger.info(MODULE, `ðŸ” CÃ³digo ingresado: ${verification_code}`);
      
      // Validar formato del cÃ³digo
      if (!verification_code || !validateCodeFormat(verification_code)) {
        Logger.error(MODULE, 'âŒ CÃ³digo invÃ¡lido (formato incorrecto)');
        return res.status(400).json({ 
          success: false, 
          error: 'CÃ³digo de 4 dÃ­gitos requerido' 
        });
      }
    ...
    const [rows] = await pool.query(
      ...
      if ((rows as any[]).length === 0) {
        Logger.error(MODULE, 'âŒ Cita no encontrada o no pertenece al proveedor');
    ...
    Logger.info(MODULE, `ðŸ” Cita encontrada: ${JSON.stringify({
      id: appointment.id,
      ...
      // Verificar que la cita tenga cÃ³digo (fue pagada)
      if (!appointment.verification_code) {
        Logger.error(MODULE, 'âŒ Cita no tiene cÃ³digo de verificaciÃ³n (no fue pagada)');
        return res.status(400).json({ 
          success: false, 
          error: 'Esta cita no tiene cÃ³digo de verificaciÃ³n. El cliente debe pagar primero.',
          meta: {
      ...
      if (appointment.verification_attempts >= 3) {
        Logger.error(MODULE, `âŒ LÃ­mite de intentos excedido (${appointment.verification_attempts})`);
        return res.status(429).json({ 
          success: false, 
          error: 'LÃ­mite de intentos excedido. Contacta a soporte.', 
          remainingAttempts: 0
        });
      }
      
      // Sanitizar y comparar cÃ³digos
      const inputCode = sanitizeCode(verification_code);
      const storedCode = sanitizeCode(appointment.verification_code);
      
      Logger.info(MODULE, `ðŸ” Comparando: '${inputCode}' vs '${storedCode}'`);
      
      if (compareVerificationCodes(inputCode, storedCode)) {
        Logger.info(MODULE, 'âœ… CÃ“DIGO CORRECTO - Marcando servicio como completado');
        
        // Marcar como completado y permitir liberaciÃ³n de fondos
        await pool.execute(
          `UPDATE appointments 
           SET status = 'completed', 
               completed_at = NOW(), 
        ...
        // Permitir liberaciÃ³n de fondos con evaluaciÃ³n de retenciÃ³n (Stripe)
        ...
        // Si ya es elegible, mover de pending_balance â†’ balance y marcar release
        try {
          ...
          if (p && Number(p.is_eligible) === 1) {
            ...
            await pool.execute(
              `INSERT INTO transactions (user_id, type, amount, currency, description, payment_id, appointment_id, created_at)
               VALUES (?, 'payment_received', ?, 'CLP', 'LiberaciÃ³n por verificaciÃ³n', ?, ?, NOW())`,
              [p.provider_id, p.provider_amount, p.id, appointmentId]
            );
            Logger.info(MODULE, 'ðŸ’¸ Fondos liberados al balance del proveedor', { paymentId: p.id, appointmentId });
          } else {
            Logger.info(MODULE, 'â³ Fondos marcados como liberables pero no elegibles aÃºn por retenciÃ³n');
          }
        } catch (fundErr) {
          Logger.warn(MODULE, 'No se pudo mover fondos al balance (se intentarÃ¡ luego por job/manual)');
        }
        ...
        Logger.error(MODULE, 'âŒ CÃ“DIGO INCORRECTO');
        ...
        // Registrar intento en tabla de auditorÃ­a (si existe)
        try {
          await pool.execute(
          ...
          // Tabla de auditorÃ­a puede no existir aÃºn
          Logger.warn(MODULE, 'Could not log verification attempt (table may not exist)');
        }
        
        const remainingAttempts = 3 - newAttempts;
        
        Logger.warn(MODULE, `âš ï¸ Intentos restantes: ${remainingAttempts}`);
        
        return res.status(400).json({ 
          success: false, 
          error: `CÃ³digo incorrecto. Intentos restantes: ${remainingAttempts}`,
          remainingAttempts
        });
      }
      
    } catch (err) {
      Logger.error(MODULE, 'ðŸ”´ Error verificando cÃ³digo', err as any);
   ...
   * Obtiene el cÃ³digo de verificaciÃ³n de una cita (solo el cliente)
   */
  router.get('/appointments/:id/verification-code', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      Logger.info(MODULE, `ðŸ” Cliente ${user.id} solicitando cÃ³digo para cita ${appointmentId}`);
      ...
      if ((rows as any[]).length === 0) {
        Logger.warn(MODULE, 'âŒ CÃ³digo no disponible (cita no existe, no pertenece al cliente, o no tiene cÃ³digo)');
        return res.status(404).json({ 
          success: false, 
          error: 'CÃ³digo no disponible. Verifica que la cita haya sido pagada.' 
        });
      }
      
      const { verification_code, code_generated_at, status } = (rows as any[])[0];
      
      Logger.info(MODULE, `âœ… CÃ³digo obtenido: ${verification_code} (generado: ${code_generated_at})`);
      ...
      Logger.error(MODULE, 'Error obteniendo cÃ³digo', err as any);
      return res.status(500).json({ 
        success: false, 
   ...
   * Lista citas pagadas del proveedor (esperando verificaciÃ³n)
   */
  router.get('/provider/appointments/paid', authenticateToken, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user || {};
      
      Logger.info(MODULE, `ðŸ’° Proveedor ${user.id} solicitando citas pagadas`);
      ...
      // Citas candidatas: (tienen cÃ³digo no vacÃ­o) OR (marcadas como cash)
      ...
      Logger.info(MODULE, `âœ… ${(rows as any[]).length} citas pagadas encontradas`);
  ...
  router.get('/provider/appointments/pending-requests', authenticateToken, async (req: Request, res: Response) => {
    try {
      ...
      console.log('[APPOINTMENTS] ðŸŽ¯ GET /provider/appointments/pending-requests para provider:', user.id);
      Logger.info(MODULE, `â³ Proveedor ${user.id} solicitando citas confirmadas sin pagar`);
      ...
      console.log('[APPOINTMENTS] ðŸ“‹ Citas confirmadas sin pagar encontradas:', (rows as any[]).length);
      Logger.info(MODULE, `âœ… ${(rows as any[]).length} citas confirmadas sin pagar encontradas`);
      ...
      console.error('[APPOINTMENTS] âŒ Error listando citas confirmadas sin pagar:', err);
   ...
   * Obtiene la prÃ³xima cita confirmada del proveedor
   */
  router.get('/provider/appointments/next', authenticateToken, async (req: Request, res: Response) => {
    try {
      const user = (req as any).user || {};
      
      console.log('[APPOINTMENTS] ðŸŽ¯ GET /provider/appointments/next para provider:', user.id);
      Logger.info(MODULE, `âž¡ï¸ Proveedor ${user.id} solicitando prÃ³xima cita confirmada`);
      ...
      console.log('[APPOINTMENTS] ðŸ“… PrÃ³xima cita encontrada:', nextAppointment ? nextAppointment.id : 'ninguna');
      Logger.info(MODULE, `âœ… PrÃ³xima cita encontrada: ${nextAppointment ? nextAppointment.id : 'ninguna'}`);
      ...
      console.error('[APPOINTMENTS] âŒ Error obteniendo prÃ³xima cita:', err);
      Logger.error(MODULE, 'Error obteniendo prÃ³xima cita', err as any);
      return res.status(500).json({ 
        success: false, 
        error: 'Error al obtener prÃ³xima cita' 
      });
    }
  });
