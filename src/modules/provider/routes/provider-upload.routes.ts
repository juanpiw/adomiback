    ...
    this.router.post(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const { url, headers } = await getPresignedPutUrl({ bucket, key, contentType, acl, expiresSeconds: 300 });
          ...
          Logger.error(MODULE, 'Error signing S3 upload', error);
    ...
    this.router.post(
      ...
      async (req: Request, res: Response) => {
        try {
          ...
          const itemId = (result as any).insertId;
          Logger.info(MODULE, 'Portfolio item finalized', { userId: user.id, itemId, key });
          return res.status(201).json({ success: true, item: { id: itemId, file_url: url, file_type: type, thumbnail_url: thumb_url || null } });
        } catch (error: any) {
          Logger.error(MODULE, 'Error finalizing portfolio item', error);
