...
const MODULE = 'ClientVerificationRoutes';
const MAX_FILE_SIZE_MB = Number(process.env.CLIENT_VERIFICATION_MAX_MB || process.env.PROVIDER_VERIFICATION_MAX_MB || 8);
...
export class ClientVerificationRoutes {
  ...
  private initialize() {
    this.router.get('/client/verification/status', authenticateToken, this.getStatus.bind(this));
    this.router.post('/client/verification/request', authenticateToken, this.createOrUpdateRequest.bind(this));
    this.router.post('/client/verification/files/sign', authenticateToken, this.signUpload.bind(this));
    this.router.post('/client/verification/files/finalize', authenticateToken, this.finalizeUpload.bind(this));
    this.router.post('/client/verification/submit', authenticateToken, this.submit.bind(this));
  }
  ...
  private async getStatus(req: Request, res: Response) {
    try {
      ...
      if (!user || user.role !== 'client') {
        return res.status(403).json({ success: false, error: 'Solo clientes pueden consultar este recurso' });
      }
  ...
  private async createOrUpdateRequest(req: Request, res: Response) {
    ...
    if (!user || user.role !== 'client') {
      return res.status(403).json({ success: false, error: 'Solo clientes pueden solicitar verificación' });
    }
    ...
    try {
      ...
      if (!existing || existing.status === 'approved') {
        const [insertResult]: any = await pool.execute(
          `INSERT INTO client_verifications
             (client_id, document_type, document_number, status, created_at, updated_at)
           VALUES (?, ?, ?, 'draft', NOW(), NOW())`,
          [user.id, document_type, document_number]
        );
        verificationId = insertResult.insertId;
        Logger.info(MODULE, 'Nueva verificación creada', { clientId: user.id, verificationId });
      } else {
        ...
        await pool.execute(
          ...
          [document_type, document_number, verificationId, user.id]
        );
        Logger.info(MODULE, 'Verificación de cliente actualizada a draft', { clientId: user.id, verificationId });
      }
      ...
      Logger.error(MODULE, 'Error creando solicitud de verificación de cliente', { error: error?.message, stack: error?.stack });
  ...
  private async signUpload(req: Request, res: Response) {
    try {
      ...
      const key = `clients/${user.id}/verification/${verificationId}/${fileTypeRaw}-${uuidv4()}.${ext}`;
      ...
      Logger.info(MODULE, 'Presign generado (cliente)', { clientId: user.id, verificationId, fileType: fileTypeRaw, key });
      ...
      Logger.error(MODULE, 'Error firmando archivo de verificación para cliente', error as any);
  ...
  private async finalizeUpload(req: Request, res: Response) {
    try {
      ...
      if (!key || !key.startsWith(`clients/${user.id}/verification/${verificationId}/`)) {
      ...
      await pool.execute(
        `INSERT INTO client_verification_files
           (verification_id, client_id, file_type, s3_bucket, s3_key, mime_type, size_bytes, checksum_sha256, uploaded_at, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW(), NOW())
      ...
      Logger.info(MODULE, 'Archivo de verificación de cliente registrado', { clientId: user.id, verificationId, fileType: fileTypeRaw });

      return res.json({ success: true });
    } catch (error) {
      Logger.error(MODULE, 'Error finalizando archivo de verificación de cliente', error as any);
  ...
  private async submit(req: Request, res: Response) {
    try {
      const user = (req as any).user as AuthUser;
      if (!user || user.role !== 'client') {
        return res.status(403).json({ success: false, error: 'Solo clientes pueden enviar verificación' });
      }
      ...
      Logger.info(MODULE, 'Verificación de cliente enviada para revisión', { clientId: user.id, verificationId });

      return res.json({ success: true, status: 'pending' });
    } catch (error) {
      Logger.error(MODULE, 'Error enviando verificación de cliente', error as any);
      return res.status(500).json({ success: false, error: 'No se pudo enviar la verificación' });
    }
  }

  private async getLatestVerification(pool: ReturnType<typeof DatabaseConnection.getPool>, clientId: number) {
    const [[row]]: any = await pool.query(
      `SELECT id, client_id, document_type, document_number, status,
  ...
  private async getVerificationById(pool: ReturnType<typeof DatabaseConnection.getPool>, verificationId: number, clientId: number) {
    const [[row]]: any = await pool.query(
      `SELECT id, client_id, document_type, document_number, status,
              rejection_reason, review_notes, reviewed_by_admin_id,
              submitted_at, reviewed_at, metadata, created_at, updated_at
    ...
    try {
      await pool.execute(
        `UPDATE users
          ...
          WHERE id = ?`,
        [status, isVerified ? 1 : 0, clientId]
      );
    } catch (error: any) {
      ...
      if (code === 'ER_BAD_FIELD_ERROR') {
        ...
        Logger.warn(MODULE, 'No se pudo sincronizar users.verification_status para cliente', {
          clientId,
          status,
          error: error?.message,
          code
...
export default new ClientVerificationRoutes().router;
